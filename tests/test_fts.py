...
class TestLessonsFTS:
    ...
    async def test_exact_title_match(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "SwiftUI State Management"})
    ...
    async def test_partial_word_match(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "animate"})
        assert len(results) > 0
    ...
    async def test_multiple_term_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "SwiftUI button"})
        assert len(results) > 0
    ...
    async def test_technology_in_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "bitbucket"})
        assert len(results) > 0
    ...
    async def test_content_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "URLSession async"})
        assert len(results) > 0
...
class TestErrorsFTS:
    ...
    async def test_error_pattern_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.search_errors({"query": "Cannot convert String Int"})
        assert len(results) > 0
    ...
    async def test_solution_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.search_errors({"query": "development team Signing"})
        assert len(results) > 0
    ...
    async def test_root_cause_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.search_errors({"query": "plist privacy"})
        assert len(results) > 0
...
class TestPatternsFTS:
    ...
    async def test_pattern_name_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.get_swift_patterns({"query": "Dependency Injection"})
        assert len(results) > 0
    ...
    async def test_description_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.get_swift_patterns({"query": "protocol abstraction"})
        assert len(results) > 0
...
class TestRelevanceRanking:
    ...
    async def test_exact_match_ranks_higher(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "Swift Concurrency"})
...
class TestCrossTableSearch:
    ...
    async def test_query_returns_all_types(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "URLSession"})
    ...
    async def test_results_sorted_by_relevance(self, knowledge_base_server):
        ...
        await knowledge_base_server.add_common_error(
            {"technology": "swift", "error_pattern": "SwiftData error", "solution": "Fix SwiftData"}
        )
        await knowledge_base_server.add_swift_pattern(
            {
        ...
        results = await knowledge_base_server.query_knowledge({"query": "SwiftData"})
...
class TestSpecialCharacters:
    ...
    async def test_quotes_in_search(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "string interpolation"})
        assert len(results) > 0
    ...
    async def test_code_snippets_searchable(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "withTaskGroup"})
        assert len(results) > 0
    ...
    async def test_api_names_searchable(self, knowledge_base_server):
        ...
        results = await knowledge_base_server.query_knowledge({"query": "URLSession shared"})
        assert len(results) > 0
